...
class ApiService {
  ...
  static Map<String, String> get authHeaders => Map.unmodifiable(_headers);
  ...
  static Future<Map<String, dynamic>> updateProfile({
    ...
    try {
      response = await http.patch(Uri.parse('$baseUrl/auth/profile'), headers: _headers, body: body);
    } catch (_) {
      response = await http.patch(Uri.parse('$fallbackBaseUrl/auth/profile'), headers: _headers, body: body);
    }
  ...
  static Future<ExamSession> createExam({
    ...
    String policyMode = 'SPECIFIC_BROWSER_NO_AI',
    String? description,
    String? browserName,
    String? allowedBrowser,
    List<String> browserExecutables = const [],
    String? aiName,
    List<String> aiDomains = const [],
    List<String> aiDesktopExecutables = const [],
    ...
    final body = <String, dynamic>{
      ...
      'browser_name': browserName ?? allowedBrowser,
      'browser_executables': browserExecutables,
      'ai_name': aiName,
      'ai_domains': aiDomains,
      'ai_desktop_executables': aiDesktopExecutables,
  ...
  static Future<List<CameraFeedItem>> fetchCameraFeed(String examId) async {
    final response = await _getRequest('/camera/exams/$examId/feed');
    if (response.statusCode == 200) {
      final List data = jsonDecode(response.body);
      return data.map((item) => CameraFeedItem.fromJson(item)).toList();
    }
    throw Exception('Failed to load camera feed');
  }

  static String cameraFrameUrl(String sessionId, {int cacheBust = 0}) =>
      '$baseUrl/camera/sessions/$sessionId/frame?v=$cacheBust';

  static Future<List<SubmissionItem>> fetchSubmissions(String examId) async {
    final response = await _getRequest('/submissions/exams/$examId');
    if (response.statusCode == 200) {
      final List data = jsonDecode(response.body);
      return data.map((item) => SubmissionItem.fromJson(item)).toList();
    }
    throw Exception('Failed to load submissions');
  }

  static Future<http.Response> downloadSubmissionZip(String examId) async {
    try {
      return await http.get(
        Uri.parse('$baseUrl/submissions/exams/$examId/download-zip'),
        headers: _headers,
      );
    } catch (_) {
      return await http.get(
        Uri.parse('$fallbackBaseUrl/submissions/exams/$examId/download-zip'),
        headers: _headers,
      );
    }
  }
