...
class StudentAttendanceItem(BaseModel):
    ...
    camera_status: str = "not_required"
    camera_frame_updated_at: Optional[datetime] = None
...
class AIResourceCreate(BaseModel):
    ...
    desktop_executables: List[str] = Field(default_factory=list, description="Optional desktop executable aliases")
    description: Optional[str] = None
...
class AccessPolicyCreate(BaseModel):
    ...
    policy_mode: PolicyMode = "SPECIFIC_BROWSER_NO_AI"
    browser_mode: str = Field(default="ALLOW_SELECTED", description="ALLOW_SELECTED, ALLOW_ANY, or BLOCK_ALL")
    ai_mode: str = Field(default="ALLOW_SELECTED", description="ALLOW_SELECTED, ALLOW_ANY, or BLOCK_ALL")
    web_access_scope: str = Field(default="ANY_SITE", description="ANY_SITE or AI_ONLY")
...
class SignedPolicyPayload(BaseModel):
    ...
    web_access_scope: str
    desktop_app_mode: str
    registered_browsers: List[dict]
    registered_ai: List[dict]
    allowed_browsers: List[str]
    allowed_ai: List[str]
    browser_ai_matrix: dict
    allowed_desktop_apps: List[str]
...
class PolicyPreviewResponse(BaseModel):
    ...
    ai_summary: List[dict]      # [{"name": "Named AI", "status": "ALLOW"}, ...]
...
class ExamSessionCreate(BaseModel):
    ...
    policy_mode: PolicyMode = "SPECIFIC_BROWSER_NO_AI"
    browser_name: Optional[str] = Field(default=None, min_length=2, max_length=80)
    browser_executables: List[str] = Field(default_factory=list)
    ai_name: Optional[str] = Field(default=None, min_length=2, max_length=80)
    ai_domains: List[str] = Field(default_factory=list)
    ai_desktop_executables: List[str] = Field(default_factory=list)
    camera_required: bool = False
    submissions_enabled: bool = True
    allowed_browser: Optional[str] = Field(default=None, description="Deprecated browser display field")
...
# --- Camera & Submission Schemas ---
class CameraStatusRequest(BaseModel):
    status: Literal["pending", "active", "denied", "unavailable"]
...
class CameraFeedItem(BaseModel):
    ...
    student_name: str
    camera_status: str
    frame_updated_at: Optional[datetime] = None
