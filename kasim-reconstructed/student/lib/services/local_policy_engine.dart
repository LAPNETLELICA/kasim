class LocalPolicyEngine {
  ...
  String get browserMode => policyPayload['browser_mode'] ?? 'ALLOW_SELECTED';
  String get aiMode => policyPayload['ai_mode'] ?? 'BLOCK_ALL';
  String get webAccessScope => policyPayload['web_access_scope'] ?? 'ANY_SITE';
  ...
  Map<String, dynamic> evaluateProcessExecutable(String exeName) {
    ...
    if (matchedBrowser == null && browserMode == 'ALLOW_ANY') {
      return {
        'permission': 'ALLOW',
        'resource_name': exeName,
        'reason': 'Any-browser mode is active',
        'type': 'BROWSER'
      };
    }
    ...
    final isAllowedBrowser = browserMode == 'ALLOW_ANY' || allowedBrowsers.contains(browserId);
  ...
  Map<String, dynamic> evaluateBrowserDomainAccess(String browserId, String domain) {
    if (browserMode != 'ALLOW_ANY' && !allowedBrowsers.contains(browserId)) {
      return {
        'permission': 'DENY',
    ...
    if (matchedAi == null) {
      return {
        'permission': webAccessScope == 'AI_ONLY' ? 'DENY' : 'ALLOW',
        'reason': webAccessScope == 'AI_ONLY'
            ? 'Standard websites are disabled in AI-only mode'
            : 'Standard web traffic (Not an AI domain)',
      };
    }
    ...
    if (aiMode == 'BLOCK_ALL') {
      return {
        'permission': 'DENY',
        'reason': 'AI access is blocked for this exam',
      };
    }

    if (aiMode != 'ALLOW_ANY' && !allowedAi.contains(aiId)) {
      return {
        'permission': 'DENY',
    ...
    final wildcardMatrix = List<String>.from(matrix['*'] ?? []);
    if (aiMode != 'ALLOW_ANY' &&
        !allowedMatrixForBrowser.contains(aiId) &&
        !wildcardMatrix.contains(aiId)) {
      return {
        'permission': 'DENY',
